...
const PHASE_BG: Record<Exclude<CyclePhase, "menstrual">, string> = {
  ...
  luteal: "bg-warning/15 text-text-secondary",
};
  ...
  return (
    <div>
      ...
      <div className="mb-3 flex flex-wrap gap-x-3 gap-y-1.5 text-[11px] font-medium text-text-secondary">
        ...
        <span className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-full bg-warning" /> {dict.cycle.calendarLegendLuteal}
        </span>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((date, i) => {
          ...
          // (chiziqcha-chiziqcha/dashed) border bilan — hayz kunining birligi
          // (solid) borderidan uslub jihatdan ajralib turadi, ikkalasi bir
          // vaqtda chizilmaydi ("bugungi kun" ustuvor).
